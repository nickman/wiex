...
/**
 * <p>Title: GroovySourceWatcher</p>
 * <p>Description: Watches a groovy source URL and issues a recompile when the source is newer than the last compile date.</p> 
...
public class GroovyScriptManager {
	 ...
	 * @throws IOException 
	 */
	public GroovyScriptManager(URL sourceUrl, Properties groovyProperties, Properties scriptProperties, Map<String, Object> args) throws IOException {
	...
	protected void init() throws IOException {
		...
		//==================================================================================================
		try {
			debug = Boolean.parseBoolean(groovyProperties.get("debug").toString());
			if(debug!=null) cc.setDebug(debug);
		} catch (Exception e) {}				
		try {
			recompile = Boolean.parseBoolean(groovyProperties.get("recomp").toString());			
		...
		try {
			tolerance = Integer.parseInt(groovyProperties.get("tolerance").toString());
			if(tolerance!=null) cc.setTolerance(tolerance);
		} catch (Exception e) {}
		try {
			verbose = Boolean.parseBoolean(groovyProperties.get("verbose").toString());
			if(verbose!=null) cc.setVerbose(verbose);
		} catch (Exception e) {}
		try {
			warning = Integer.parseInt(groovyProperties.get("warning").toString());
			if(warning!=null) cc.setWarningLevel(warning);
		} catch (Exception e) {}
		long start = System.currentTimeMillis();
		for(Entry<Object, Object> entry: scriptProperties.entrySet()) {
